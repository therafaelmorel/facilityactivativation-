...
const adminIconClasses={projects:'ph-buildings',plus:'ph-plus',library:'ph-books',tasks:'ph-check-square-offset',users:'ph-users-three',history:'ph-clock-counter-clockwise'};
  ...
  return `<div class="admin-dashboard">
    <aside class="admin-sidebar">
      ...
      <nav class="admin-nav" aria-label="Administrator navigation">
        ${r.nav.map(([icon,label,screen])=>`<button type="button" class="admin-nav-link ${active===screen?'active':''}" data-screen="${screen}">
          <i class="ph ${adminIconClasses[icon]||'ph-squares-four'}"></i>
          <span>${label}</span>
        </button>`).join('')}
      </nav>
      <div class="admin-access">
        <i class="ph ph-shield-check"></i>
        <span><strong>Administrator access</strong><small>Create, assign, edit and publish projects.</small></span>
      </div>
      <div class="admin-profile">
        <span class="admin-avatar">${r.initials}</span>
        <span><strong>${r.name}</strong><small>${r.title}</small></span>
        <button type="button" data-screen="sign-in" aria-label="Sign out"><i class="ph ph-sign-out"></i></button>
      </div>
    </aside>
    <main class="admin-workspace">
      <header class="admin-topbar">
        ...
        <div class="admin-top-actions">
          ...
          <button type="button" class="admin-search-trigger" aria-label="Search"><i class="ph ph-magnifying-glass"></i></button>
          <button type="button" class="admin-notification" aria-label="Notifications"><i class="ph ph-bell-simple"></i><b>3</b></button>
          <span class="admin-avatar">${r.initials}</span>
        </div>
      </header>
...
function layout(title,active,body){
  if(currentRole==='admin')return adminLayout(title,active,body);
  const r=role();
  return `<div class="shell"><aside class="sidebar"><div class="product"><div class="product-mark">FA</div><div><div class="product-name">Facility Activation</div><div class="product-sub">Readiness workspace</div></div></div><div class="role-context"><span>Current workspace</span><strong>${r.label}</strong><small>${r.description}</small></div><nav class="nav">${r.nav.map(n=>`<button type="button" class="nav-link ${active===n[2]?'active':''}" data-screen="${n[2]}">${I(n[0])}<span>${n[1]}</span></button>`).join('')}</nav><div class="permission-note"><span>Access</span><strong>${r.label} permissions</strong><small>${currentRole==='team'?'Update status, notes and evidence only.':'Read-only portfolio and project visibility.'}</small></div><div class="sidebar-foot"><div class="user-mini"><div class="avatar">${r.initials}</div><div><strong>${r.name}</strong><span>${r.title}</span></div></div></div></aside><main class="main"><div class="demo-bar"><div><span class="live-dot"></span><strong>Interactive role demo</strong><span class="demo-copy">Select a role to open its dashboard and permissions</span></div>${roleTabs()}</div><header class="topbar"><div class="crumb">Facility Activation <span>/</span> <strong>${title}</strong></div><div class="top-actions"><button type="button" class="text-action" data-screen="sign-in">Choose role</button><button type="button" class="text-action">Notifications <span class="notification-count">3</span></button><div class="avatar">${r.initials}</div></div></header><div class="content">${body}</div></main></div>`;
}
...
function wireAdminInteractions(){
  const search=document.querySelector('.admin-search-trigger');
  if(search)search.onclick=()=>toast('Search administrator workspace — demo action');
  const notification=document.querySelector('.admin-notification');
  if(notification)notification.onclick=()=>toast('3 new administrator notifications');
  document.querySelectorAll('.admin-content .choice').forEach(choice=>choice.onclick=()=>{
    const enabled=choice.classList.toggle('yes');
    const value=choice.querySelector('strong');
    if(value)value.textContent=enabled?'Yes':'No';
  });
  document.querySelectorAll('.admin-content .dept-row').forEach(row=>row.onclick=()=>{
    document.querySelectorAll('.admin-content .dept-row').forEach(item=>item.classList.remove('active'));
    row.classList.add('active');
    const heading=document.querySelector('.assignment-main h2');
    const summary=document.querySelector('.assignment-main h2 + .sub');
    const name=row.querySelector('strong')?.textContent;
    const count=row.querySelector('span')?.textContent;
    if(heading&&name)heading.textContent=name;
    if(summary&&count)summary.textContent=`${count.replace(' tasks','')} accountable tasks · 4 unassigned`;
  });
  document.querySelectorAll('.admin-content .tab').forEach(tab=>tab.onclick=()=>{
    const label=tab.textContent.trim();
    if(label==='Tasks'){navigate('project-tasks');return}
